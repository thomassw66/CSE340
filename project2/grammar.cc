...
{
	if (rule_list.size() == 0)
		startSymbol = r->getLHS();

	addSymbol(r->getLHS());
	for (int i = 0; i < r->getRHSSize(); i++) {
		string rhs = r->getRHS(i);
		addSymbol(rhs);
	}
	rule_list.push_back(r);
}

void Grammar::addSymbol(string id)
{
	universe.insert(id);
}

set<string> * Grammar::getGeneratingSymbols() 
{
	set<string> * generating = new set<string>();
	...
	while (f) {
		...
		for (Rule * r : rule_list) {
			string lhs = r->getLHS();
			// Variable is not yet generating
			if (generating->find(lhs) == generating->end()) {
				bool g = true;
				for (int i = 0; i < r->getRHSSize(); i++) {
					string el = r->getRHS(i);
					bool isGenerating = generating->find(el) != generating->end();
					g = g && isGenerating;
				}
				if (g) { // every symbol of the rhs is generating therefore the variable must generate
...
set<string> * Grammar::getReachableSymbols() 
{
	set<string> * generating = getGeneratingSymbols();
	set<string> * reachable = new set<string>();
	queue<string> * queue = new queue<string>();

	queue->push_back(startSymbol);
	while(queue->size() > 0) {
		string n = queue->pop_front();
		if (reachable->find(n) == reachable->end()) {
			reachable->insert(n);
			for (Rule * r : rule_list) {
				if (r->getLHS() == n) {
					for (int i = 0; i < r->getRHSSize(); i++) {
						string id = r->getRHS(i);
						queue->push_back(id);
						if (generating->find(id) == generating->end()) break;
					}
				}
			}
		}
...
set<string> * Grammar::getFirstSet(string id)
{
	set<string> * s = new set<string>(); 
	for (string str : universe) {
		// s->insert(str);
	}
	return s; 
}

set<string> * Grammar::getFollowSet(string id) 
{
	set<string> * s = new set<string>(); 
	for (string str : universe) {
		// s->insert(str);
...
bool Grammar::isTerminal(string id) 
{
	// linear search through rules .. if id is a lhs of a rule then it is not terminal
	for (Rule * r : rule_list) {
		if (r->getLHS() == id) {
			return false;
		}
